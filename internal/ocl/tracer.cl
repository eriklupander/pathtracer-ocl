...
// randomVectorInHemisphere is based on https://raytracey.blogspot.com/2016/11/opencl-path-tracing-tutorial-2-path.html but adapted to use another
// rand function and double4 instead of float4.
...
// I think randomConeInHemisphere distributes the rays more "cone-like" while this one distributes them better across the entire
...
inline double4 mul(double16 mat, double4 vec) {
    ...
    return (double4)(
        elem1.x + elem1.y + elem1.z + elem1.w,
    ...
    for (unsigned int samples = 0; samples < SAMPLE_COUNT;samples++) {
        ...
        for (unsigned int b = 0; b < MAX_BOUNCES; b++) {
            ...
            for (unsigned int j = 0; j < numObjects; j++) {
                ...
                if (objType == 0) {  // intersect transformed ray with plane
                    ...
                    if (fabs(tRayDirection.y) > 0.0001) {
                        ...
                        intersections[j] = t;
                        //intersection ixs = {0, t};
                        //xs[numIntersections] = ixs;
                        numIntersections++;
                    }
                }
            ...
            if (lowestIntersectionIndex > -1) {
                ...
                // The vector to the eye (or last bounce origin) is exactly the opposite of the ray direction
                double4 eyeVector = -rayDirection;

                // object normal at intersection: Transform point from world to object space
                ...
                // The "inside" stuff from the old impl will be needed for refraction later
                // comps.Inside = false
                // negate the normal if the normal if facing away from the "eye"
                double4 normalFacing; // = dot(eyeVector, normalVec) < 0 ? normalVec : normalVec * (-1.0f);
                ...
                // Compute the over point, with a slight offset along the normal, in order to avoid self-intersection
                // on the next bounce.
            ...
            //  else {
            //      printf("a ray did not intersect anything, number of bounces: %d\n.", actualBounces);
            //      printf("the actual bounce: %f %f %f\n", bounces[b].color.x, bounces[b].color.y, bounces[b].color.z);
            // }
        }
        ...
        for (unsigned int x = 0; x < actualBounces; x++) {
            ...
            // First, ADD current accumulated color with the hadamard of the current mask and the emission properties of the hit object.
            ...
            // perform cosine-weighted importance sampling by multiplying the mask with the cosine
            mask *= bounces[x].cos;
        }
