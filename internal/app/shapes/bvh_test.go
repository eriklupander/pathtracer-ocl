...
func TestSplitPerfectCube(t *testing.T) {
	...
	assert.Equal(t, geom.NewPoint(-1, -4, -5), left.Min)
	assert.Equal(t, geom.NewPoint(4, 6, 5), left.Max)
	assert.Equal(t, geom.NewPoint(4, -4, -5), right.Min)
	assert.Equal(t, geom.NewPoint(9, 6, 5), right.Max)
}
...
func TestSplitXWideBoundingBox(t *testing.T) {
	...
	left, right := SplitBounds(box)
	assert.Equal(t, geom.NewPoint(-1, -2, -3), left.Min)
	assert.Equal(t, geom.NewPoint(4, 5.5, 3), left.Max)
	assert.Equal(t, geom.NewPoint(4, -2, -3), right.Min)
	assert.Equal(t, geom.NewPoint(9, 5.5, 3), right.Max)
}
...
func TestSplitYWideBoundingBox(t *testing.T) {
	...
	left, right := SplitBounds(box)
	assert.Equal(t, geom.NewPoint(-1, -2, -3), left.Min)
	assert.Equal(t, geom.NewPoint(5, 3, 3), left.Max)
	assert.Equal(t, geom.NewPoint(-1, 3, -3), right.Min)
	assert.Equal(t, geom.NewPoint(5, 8, 3), right.Max)
}
func TestSplitZWideBoundingBox(t *testing.T) {
	...
	left, right := SplitBounds(box)
	assert.Equal(t, geom.NewPoint(-1, -2, -3), left.Min)
	assert.Equal(t, geom.NewPoint(5, 3, 2), left.Max)
	assert.Equal(t, geom.NewPoint(-1, -2, 2), right.Min)
	assert.Equal(t, geom.NewPoint(5, 3, 7), right.Max)
}
...
//	s1 := NewSphere()
//	s1.SetTransform(geom.Translate(-1.5, 0, 0))
//	s2 := NewSphere()
//	s2.SetTransform(geom.Translate(1.5, 0, 0))
//	s3 := NewSphere()
//	s3.SetTransform(geom.Translate(0, 0, -1.5))
//	s4 := NewSphere()
//	s4.SetTransform(geom.Translate(0, 0, 1.5))
//
//	// groups
//	left := NewGroup()
