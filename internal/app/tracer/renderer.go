...
func (ctx *Ctx) renderPixelPathTracer(width, height int) {
	...
	// Render the scene
	result := ocl.Trace(sceneObjects, triangles, groups, cmd.Cfg.DeviceIndex, height, ctx.samples, clCamera, ctx.scene.Textures, ctx.scene.SphereTextures, ctx.scene.CubeTextures)
